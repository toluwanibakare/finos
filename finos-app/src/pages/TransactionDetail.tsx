...
const statusConfig: Record<string, { label: string; icon: string; color: string; bg: string }> = {
  completed: { label: 'Completed', icon: 'check-circle', color: 'text-emerald-600', bg: 'bg-emerald-50' },
  pending: { label: 'Pending', icon: 'clock', color: 'text-amber-600', bg: 'bg-amber-50' },
  processing: { label: 'Processing', icon: 'loader', color: 'text-blue-600', bg: 'bg-blue-50' },
  failed: { label: 'Failed', icon: 'x-circle', color: 'text-red-500', bg: 'bg-red-50' },
}
    ...
    <PageContainer>
      ...
      <div className="pt-4 space-y-4">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center animate-fade-in">
          <p className={`text-3xl font-bold tracking-tight ${isCredit ? 'text-emerald-600' : 'text-[#0A1628]'}`}>
            {isCredit ? '+' : txn.type === 'allocation' ? '' : '-'}{formatNaira(txn.amount)}
          </p>
          <p className="text-[13px] text-gray-400 mt-1">{txn.description}</p>
          <div className="flex items-center justify-center gap-2 mt-3">
            <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full ${status.color} ${status.bg}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in" style={{ animationDelay: '80ms' }}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in" style={{ animationDelay: '160ms' }}>
          <div className="px-5 pt-4 pb-2">
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Info</p>
          </div>
          <div className="divide-y divide-gray-50">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold text-[#0A1628]">{relatedIncome.description}</p>
                  <p className="text-[11px] text-gray-400">{relatedIncome.reference}</p>
                </div>
...
function DetailRow({ label, value, onClick }: { label: string; value: string; onClick?: () => void }) {
  return (
    <div
      className={`flex items-center justify-between px-5 py-3.5 ${onClick ? 'active:bg-gray-50 cursor-pointer' : ''}`}
      onClick={onClick}
    >
      <span className="text-[12px] text-gray-400 font-medium">{label}</span>
      <span className="text-[13px] font-medium text-[#0A1628] text-right max-w-[60%] break-words">
        {value}
        {onClick && <FinosIcon name="chevron-right" size={14} className="text-gray-300 ml-1 inline" />}
      </span>
    </div>
  )
}
