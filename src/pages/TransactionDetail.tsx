  ...
  return (
    <PageContainer>
      ...
      <div className="pt-4 space-y-4">
        {/* Amount header */}
        <div className="bg-white rounded-[20px] p-6 text-center animate-fade-in">
          <p className={`text-[32px] font-bold tracking-[-0.03em] tabular-nums ${isCredit ? 'text-[#2E7D32]' : 'text-[#0B1320]'}`}>
        ...
        {/* Details */}
        <div className="bg-white rounded-[20px] overflow-hidden animate-fade-in" style={{ animationDelay: '80ms' }}>
        ...
        {/* Info */}
        <div className="bg-white rounded-[20px] overflow-hidden animate-fade-in" style={{ animationDelay: '160ms' }}>
          <div className="px-5 pt-4 pb-2">
        ...
        {relatedIncome && (
          <div className="animate-fade-in" style={{ animationDelay: '240ms' }}>
            ...
            <button
              ...
              className="w-full bg-white rounded-[16px] p-4 text-left active:bg-gray-50 transition-colors duration-150"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-bold text-[#0B1320]">{relatedIncome.description}</p>
...
function DetailRow({ label, value, onClick }: { label: string; value: string; onClick?: () => void }) {
  return (
    <div
      className={`flex items-center justify-between px-5 py-3.5 ${onClick ? 'active:bg-gray-50 cursor-pointer' : ''} transition-colors duration-150`}
      onClick={onClick}
    >
      <span className="text-[12px] text-gray-400 font-medium">{label}</span>
      <span className="text-[13px] font-semibold text-[#0B1320] text-right max-w-[60%] break-words">
        {value}
